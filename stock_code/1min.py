...
class CpStockChart:
    ...
    def RequestMinuteData(self, code, targetDate, caller):
        # 데이터 초기화 (변수명 5분봉 코드와 동일하게 맞춤)
        caller.dates, caller.times, caller.opens, caller.highs = [], [], [], []
        caller.lows, caller.closes, caller.vols, caller.turnovers = [], [], [], []
        caller.bid_sizes, caller.ask_sizes = [], [] # 매수/매도 잔량(누적체결량) 리스트

        print(f"[{code}] 1분봉 조회 시작: 20251230부터 {targetDate}까지")
        ...
        # ★★★ 핵심 변경: 5분 -> 1분 ★★★
        self.objStockChart.SetInputValue(7, 1)        
...
class MyWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("전종목 1분봉 수집기 (구조 통일)")
        self.resize(400, 200)

        layout = QVBoxLayout()
        self.btn_auto = QPushButton("1분봉 전체 수집 시작")
    ...
    def start_batch_process(self):
        # 파일명은 사용하시는 것에 맞춰 변경 (예: KOSPI200)
        ...
        for code in codes:
            ...
            # 목표 날짜 설정 (1분봉이므로 데이터량이 많아 기간 조절 필요 시 수정)
            self.objChart.RequestMinuteData(str_code, 20250101, self)
    ...
    def save_to_csv(self, code):
        ...
        filename = f"KOSDAQ_A{clean_code}_1min_data.csv"
        
        # 1. 초기 DataFrame 생성 (5분봉 코드와 컬럼명/구조 100% 동일)
        df = pd.DataFrame({
            'ticker': clean_code,
