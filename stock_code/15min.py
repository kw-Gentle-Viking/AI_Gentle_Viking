...
class CpStockChart:
    ...
    def RequestMinuteData(self, code, targetDate, caller):
        ...
        caller.bid_sizes, caller.ask_sizes = [], [] # 매수/매도 잔량(누적체결량)

        print(f"[{code}] 15분봉 조회 시작: 20251230부터 {targetDate}까지")

        self.objStockChart.SetInputValue(0, code)
        self.objStockChart.SetInputValue(1, ord('1')) # 기간 기준
        self.objStockChart.SetInputValue(2, 20251230) # 수집 종료일(최신)
        self.objStockChart.SetInputValue(3, targetDate) # 수집 시작일(과거)
        
        # 0:일자, 1:시간, 2:시가, 3:고가, 4:저가, 5:종가, 8:거래량, 9:거래대금, 10:누적매도수량, 11:누적매수수량
        # [구조 통일] 매수/매도 체결량 필드(10, 11) 추가
        self.objStockChart.SetInputValue(5, [0, 1, 2, 3, 4, 5, 8, 9, 10, 11]) 
        self.objStockChart.SetInputValue(6, ord('m')) # 분봉
        
        # ★★★ 핵심 설정: 15분봉 ★★★
        self.objStockChart.SetInputValue(7, 15)        
        ...
        while True:
            ...
            for i in range(count):
                ...
                # [구조 통일] 요청 리스트 인덱스 8, 9에 해당하는 값 수집
                caller.ask_sizes.append(self.objStockChart.GetDataValue(8, i)) # 필드 10 (누적매도)
                caller.bid_sizes.append(self.objStockChart.GetDataValue(9, i)) # 필드 11 (누적매수)
...
class MyWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("전종목 15분봉 수집기 (구조 통일)")
        self.resize(400, 200)

        layout = QVBoxLayout()
        self.btn_auto = QPushButton("15분봉 전체 수집 시작")
    ...
    def start_batch_process(self):
        ...
        for code in codes:
            ...
            # [요청] 기간: 20230102 까지 (약 2년치)
            self.objChart.RequestMinuteData(str_code, 20230102, self)
    ...
    def save_to_csv(self, code):
        ...
        filename = f"KOSPI_A{clean_code}_15min_data.csv"
        
        # 1. 초기 DataFrame 생성 (컬럼명 영문 통일, ticker 포함)
        df = pd.DataFrame({
            'ticker': clean_code,
            ...
            'bid_size_total': self.bid_sizes, # 누적 매수량
            'ask_size_total': self.ask_sizes  # 누적 매도량
        })
